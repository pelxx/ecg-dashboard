...
// Semua logika utama ada di dalam komponen ini
function PatientPageContent() {
  ...
  useEffect(() => {
    ...
    recentData.forEach(point => {
        if (point.value > R_PEAK_THRESHOLD && (point.timestamp - (lastPeakTimeRef.current[selectedKey] || 0)) > MIN_INTERVAL_MS) {
  ...
  const handleManualSave = async (deviceId: string) => {
    ...
    if (!currentData || currentData.lead1.length === 0) return alert("Tidak ada data live untuk disimpan.");
    try {
      const newRecordRef = push(ref(rtdb, `ecg/records`));
      await set(newRecordRef, { createdAt: Date.now(), patientId: deviceId, note: `Manual save`, data: currentData });
                    ...
                    <div className="bg-gray-900 p-4 rounded-lg border border-blue-800/50">
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setYAxisMode('fixed')} className={`px-2 py-1 text-xs rounded ${yAxisMode === 'fixed' ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-300'}`}>Fixed</button>
                            </div>
                        </div>
  ...
  // PERBAIKAN: Mengembalikan JSX yang benar untuk daftar pasien
  return (
    <main className="min-h-screen bg-black text-white p-6">
...
// Komponen utama sekarang menjadi pembungkus Suspense
